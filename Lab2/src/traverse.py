...
import os, numpy, math, csv, time, datetime
...
def histogram(dataset, xlabel, title):
		...
		pyplot.hist(dataset,bins=25, log=True)
		#pyplot.plot(xgrid, density.evaluate(xgrid), label='kde', color='g')
		#pyplot.plot(xgrid, stats.norm.pdf(xgrid), label='DGP normal', color="r")
...
if __name__ == '__main__':
	...
	if arguments['--csv']:
		...
		filename = "data/traverse_" + str(datetime.datetime.now()).split('.')[0] + ".csv"
